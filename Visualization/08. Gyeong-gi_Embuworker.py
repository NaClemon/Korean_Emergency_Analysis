...
data = []
for si in index:
    noEmbul = state_data[state_data['시군명'] == si]['구급대원수(명)'].sum()
    data.append(noEmbul)
...
myData = pd.DataFrame(data=datas, index=range(len(index)), columns=['시군명', '구급대원수(명)'])
myData = myData.astype({'구급대원수(명)': 'int'})
print(myData)
...
m.save('Result/5-Gyeong-gi_Embuworker.html')